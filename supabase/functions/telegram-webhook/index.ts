...
Deno.serve(async (req) => {
  ...
  if (!sessionId) {
    return jsonResponse({ error: "failed to upsert session" }, { status: 500 });
  }

  // 2) Insert inbound message (idempotent on channel_update_id)
  const { error: mErr } = await supabase.from("messages").insert({
    ...
    content,
    channel: "telegram",
    channel_update_id: updateId,
    channel_message_id: messageId,
    channel_chat_id: chatId,
    channel_from_user_id: fromUserId,
  });

  // If we hit a unique violation (duplicate update), we can safely continue.
  if (mErr && mErr.code !== "23505") {
    return jsonResponse({ error: mErr.message }, { status: 500 });
  }
