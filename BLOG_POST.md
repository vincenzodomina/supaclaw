...
Looks like always-on agent are THE thing now. OpenClaw and its community proved that the proactive AI worker is not science fiction anymore. The era of digital employees has started, and with ideas like company-as-code floating around, this is only going to accelerate.
...
I use Supabase daily. I know it. I trust it. I'm already self-hosting it for production projects. It already ships every primitive an always-on agent needs.
...
What is it actually that makes OpenClaw feel like magic:
...
- Memory across conversations & devices

Now look at what Supabase gives you out of the box:

- Postgres for data + Autogenerated API and SDK
...
- Vault for secrets
- A full blown admin dashboard UI to manage it all

It maps almost one-to-one. Every agent concept has a battle-tested Supabase feature behind it. No glue infrastructure needed.

And yes, I know that local filesystem access is what makes agents truly magical — rummaging through your computer, finding old files, using the terminal to fix things and even config and improve itself. But let's be honest, 80% or more of my use cases are online knowledge work. Research, brainstorming, writing, working with a curated set of files for a specific project. For that, a cloud-based agent is more than enough.

This is also not for coding. I still use Cursor and terminal-based agents for that. They're better suited.
...
Edge Functions handle the HTTP surface. One for Telegram webhooks, one for the agent worker, one for external triggers. That's the entire deployment.
...
Let me walk you through actual code. I kept the snippets minimal so you can skim fast.
...
But there are a few crazy things I'm thinking about:
